...
from bin.data_process1 import em_impute_numerical, em_impute_categorical
import lib
...
class T2GFormer(nn.Module):
    ...
    def forward(self, x_num: Tensor, x_cat: ty.Optional[Tensor], return_fr: bool = False) -> Tensor:
        ...
        missing_mask_num = torch.isnan(x_num)
        missing_mask_cat = (x_cat == -1)
        if torch.any(missing_mask_num) or torch.any(missing_mask_cat):
            # Impute missing values using EM algorithm
            imputed_x_num = em_impute_numerical(x_num, missing_mask_num, max_iterations=10)
            imputed_x_cat = em_impute_categorical(x_cat, missing_mask_cat, max_iterations=10)
        else:
            # No missing values, use the original data
            imputed_x_num = x_num
            imputed_x_cat = x_cat
        x = self.tokenizer(imputed_x_num, imputed_x_cat)
