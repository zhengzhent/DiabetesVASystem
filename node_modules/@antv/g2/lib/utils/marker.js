...
function useMarker(type, _a) {
    var { d, fill, strokeWidth, path, stroke, lineWidth, color } = _a, style = __rest(_a, ["d", "fill", "strokeWidth", "path", "stroke", "lineWidth", "color"]);
    const symbol = exports.Symbols.get(type) || exports.Symbols.get('point');
    return (...args) => {
        const path = new g_1.Path({
            style: Object.assign(Object.assign({}, style), { path: symbol(...args), stroke: symbol.style.includes('stroke') ? color || stroke : '', fill: symbol.style.includes('fill') ? color || fill : '', lineWidth: symbol.style.includes('lineWidth')
